...
def test_validate_hostname_normalizes():
    assert (
        dns_svcb_info._validate_hostname(" Example.COM. ")
        == "example.com"
    )
...
def test_validate_hostname_rejects_invalid(hostname: str):
    with pytest.raises(ValueError):
        dns_svcb_info._validate_hostname(hostname)
...
def test_resolve_svcb_records(monkeypatch):
    monkeypatch.setattr(
        dns_svcb_info.dns.resolver,
        "Resolver",
        lambda: FakeResolver(),
    )

    assert dns_svcb_info.resolve_svcb_records("Example.COM.") == [
        {
            "priority": 0,
            "target": ".",
            "record": "0 .",
...
def test_resolve_svcb_records_dns_error(monkeypatch):
    class ErrorResolver:
        def resolve(self, hostname: str, record_type: str):
            raise dns_svcb_info.dns.exception.DNSException()
