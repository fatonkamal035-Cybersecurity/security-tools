"""Read-only DNS RP (Responsible Person) record lookup."""
...
def resolve_rp_records(
    hostname: str,
) -> list[dict[str, str]]:
    """Resolve RP records for one explicitly provided hostname."""
    ...
    records = {
        (
            str(answer.mbox).rstrip(".").lower(),
            str(answer.txt).rstrip(".").lower(),
        )
    ...
    return [
        ...
        for mailbox, txt_name in sorted(records)
    ]
